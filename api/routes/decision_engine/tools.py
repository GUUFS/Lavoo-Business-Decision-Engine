...
from decision_engine.analyst_db import analyze_business_needs
from decision_engine.recommender_db import recommend_tools
from decision_engine.utils.comparison_db import compare_tools_db
...
async def get_recommendations(query: str, top_k: int = 3, db: Session = Depends(get_db)):
    """
...
async def get_business_analysis(user_role: str, db: Session = Depends(get_db)):
    """
...
async def get_tool_comparison(tools: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if not comparison:
            raise HTTPException(status_code=404, detail="No tools found for comparison")
...
async def search_tools(query: str, limit: int = 10, db: Session = Depends(get_db)):
    """
...
async def get_tool_details(tool_id: int, db: Session = Depends(get_db)):
    """
